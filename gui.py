# gui.py (Version Diagnostic avec Barre d'État)
...
class GameGUI(tk.Frame):
    ...
    def new_game(self, ai, diff="mittel"):
        try:
            ...
            msg = "Partie lancée. " + ("Tu es les ROUGES." if ai else "Les ROUGES commencent.")
            self.log(msg + " Place ta TOUR dans un COIN (Click case 1,1).")
            
        except Exception as e:
            messagebox.showerror("Erreur New Game", str(e))

    def update_title(self):
        mode = f"KI-{self.ki_difficulty}" if self.ki_active else "PvP"
        turn = "Rot" if self.game.turn == 1 else "Blau"
    ...
    def on_click(self, event):
        try:
            ...
            if (r, c) == self.game.pfarrer_pos:
                ...
                self.log("Pfarrer sélectionné ! Clique maintenant sur une pièce BLOQUÉE pour échanger.")
                self.draw_board()
            ...
            if clicked_piece and clicked_piece.spieler == self.game.turn:
                ...
                if self.selected_pos == self.game.pfarrer_pos:
                    ...
                    if swap_move:
                        ...
                        self.log("Cette pièce n'est pas bloquée ou échange impossible.")
    ...
    def execute_move(self, move):
        self.log(f"Exécution coup: {move['type']}")
        self.game = self.game.apply_move(move)
        self.draw_board()
        self.update_title()
        
        if self.game.game_over:
            w = "Rot" if self.game.winner == 1 else "Blau"
            messagebox.showinfo("Ende", f"{w} gewinnt!")
            return

        if self.ki_active and self.game.turn == 2:
            self.log("L'IA réfléchit...")
            self.master.after(100, self.run_ai)
        else:
            self.log("À toi de jouer.")
    ...
    def run_ai(self):
        try:
            ...
            if move:
                ...
                self.update_title()
                if self.game.game_over:
                    messagebox.showinfo("Ende", f"KI ({self.ki_difficulty}) gewinnt!")
                else:
                    self.log("L'IA a joué. À toi.")
            else:
                self.log("L'IA passe son tour (bloquée).")
        except Exception as e:
            traceback.print_exc()
